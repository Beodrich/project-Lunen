    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        else if ( property.propertyType == SerializedPropertyType.Vector2Int )
		{
			...
			if ( minMax.ShowDebugValues || minMax.ShowEditRange )
			{
				...
				val = EditorGUI.Vector2Field ( position, "Min/Max Value", val );

				if ( isEditable )
				{
					property.vector2IntValue = new Vector2Int ( Mathf.RoundToInt ( val.x ), Mathf.RoundToInt ( val.y ) ); // save off any change to the value~
				}
			}
		}
    }
